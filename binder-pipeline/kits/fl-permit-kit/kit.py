"""Shared furniture and helpers for the Florida Owner-Builder Permit Kit.
...
SECTION = "Florida Owner-Builder Permit Kit"
HEADER_LEFT = "FLORIDA OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Florida's owner-builder disclosure statement is twelve
    numbered paragraphs the statute prints in full, and a 250pt callout
    arriving two thirds down a page throws the whole rest of the page away —
    several pages of pure whitespace across the kit. This rebuilds the same
    box as a LongTable that breaks between body paragraphs.
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Florida is the "
           "opposite of a no-permit state: one building code binds every "
           "parcel in all 67 counties, every jurisdiction permits and "
           "inspects, and a local government may not simply write its own "
           "residential code. What still varies is the part that costs you "
           "money and weeks — fees, impact fees, review queues, portals, and "
           "which <i>additional</i> permits your particular lot triggers "
           "before the building department will act. Confirm every item with "
           "the office that will actually handle your parcel before you rely "
           "on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Florida claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Every source cited in this kit is free to read. A <b>Fla. Stat.</b> "
    "section is at <b>flsenate.gov</b> → Laws → Florida Statutes, or go "
    "straight to the section by typing the year and number into the address "
    "bar — <i>flsenate.gov/Laws/Statutes/2024/489.103</i> — which is faster "
    "than the search box (chapters used here: <b>489</b> contracting, "
    "<b>553</b> the building code, <b>713</b> construction liens, <b>161</b> "
    "coastal, <b>373</b> water). A rule cited as <b>F.A.C.</b> is at "
    "<b>flrules.org</b> → Search → by rule number. The building code itself "
    "is at <b>floridabuilding.org</b> → Codes, free to read online without an "
    "account. <b>One warning when you check a number:</b> flsenate.gov keeps "
    "every past year's statutes at a nearly identical address, so a search "
    "result can hand you a repealed version — check that the year in the URL "
    "is the current one before you rely on the text. All text here was read "
    "at those sources, September 2026.")
...
def closing_note(text=None):
    ...
    strands its tail on a sheet of its own — which is how AK.2 acquired a
    twelfth page carrying one sentence. KeepTogether moves the whole note
