#!/usr/bin/env python3
"""Output audit for the LOUISIANA Permit Kit PDFs.
...
Catches the four defects that survive a clean build but ruin a printed page:
  ...
  - words broken mid-token by a too-narrow table column ("lslbc.louisiana.g /
...
OUT = os.path.join(HERE, "out", "la-permit-kit")
...
def audit(pdf):
    ...
    for i, ws in enumerate(pages, start=1):
        ...
        is_cover = name.startswith("LA.0") and i == 1
