"""Shared furniture and helpers for the Louisiana Owner-Builder Permit Kit.
...
SECTION = "Louisiana Owner-Builder Permit Kit"
HEADER_LEFT = "LOUISIANA OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Louisiana's statutory quotations run long enough that a
    ...
    to the first body paragraph so a title can never strand at the foot of a
    page.
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Louisiana writes one "
           "mandatory construction code for the whole state and then hands "
           "enforcement to each parish and municipality, which is why the code "
           "your house must meet is the same in Cameron Parish as in Caddo "
           "while the office that inspects it, the forms it hands you and the "
           "fee it charges are not. Confirm every local item with the office "
           "that will actually handle your parcel before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Louisiana claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Everything cited here is free to read. A <b>R.S.</b> citation is a "
    "Louisiana Revised Statute — search the number at <b>legis.la.gov</b> "
    "under Louisiana Laws. A <b>LAC</b> citation is the Louisiana "
    "Administrative Code, published as free PDFs by the Office of the State "
    "Register at <b>doa.la.gov</b>. Rules newer than the last codified "
    "printing appear only in the <i>Louisiana Register</i> at the same site, "
    "so check there before relying on a date. Read September 2026.")
...
def closing_note(text=None):
    ...
    Watch the page this lands on. Because the note can only move as a unit, a
    document whose sources table happens to fill to the foot of a page will
    push the note onto a sheet of its own carrying five lines of italic and
    nothing else — LA.2 and LA.4 both did, and both were fixed by trimming a
    write-in row so the pair fits together. Binding the table and the note in
    one KeepTogether is NOT the fix: it moves the whole table instead and
    strands the previous page.
    """
    from reportlab.platypus import KeepTogether
