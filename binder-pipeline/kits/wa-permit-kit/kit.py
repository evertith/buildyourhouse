"""Shared furniture and helpers for the Washington Owner-Builder Permit Kit.
...
SECTION = "Washington Owner-Builder Permit Kit"
HEADER_LEFT = "WASHINGTON OWNER-BUILDER PERMIT KIT"
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Washington's "
           "building code is written by the State and enforced locally by 39 "
           "counties and hundreds of cities and towns, each with its own "
           "forms and fees — and your electrical permit may come from a "
           "different agency altogether. Confirm every item with the "
           "department that will issue your permit before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Washington claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any statute cited above at <b>app.leg.wa.gov/RCW/default.aspx?"
    "cite=18.27.090</b>, and any rule at <b>app.leg.wa.gov/WAC/default.aspx?"
    "cite=296-46B-925</b>, with the chapter and section swapped in. All RCW "
    "and WAC text in this kit was read there and verified August 2026.")
