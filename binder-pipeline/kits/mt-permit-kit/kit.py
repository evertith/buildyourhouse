"""Shared furniture and helpers for the Montana Owner-Builder Permit Kit.
...
SECTION = "Montana Owner-Builder Permit Kit"
HEADER_LEFT = "MONTANA OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    """A callout that MAY split, repeating nothing but its border.

    callout() wraps design.callout_box in KeepTogether, which is right for a
    short advisory. Montana's exemption statutes are quoted at length — a
    260pt box arriving two thirds down a page throws the rest of the page
    away. This rebuilds the same box as a LongTable that breaks between body
    paragraphs, with the title row glued to the first paragraph so a title can
    never strand at the foot of a page. Lifted from the California kit.
    """
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Montana's building "
           "code does not apply to a residential building of fewer than five "
           "dwelling units unless the local legislative body has adopted it "
           "(50-60-102(1)(a), MCA) — but the electrical permit, the energy "
           "code, and all of water and wastewater law survive that exemption, "
           "on separate tracks with separate offices. Confirm every item with "
           "the office that will actually issue it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Montana claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any <b>MCA</b> section cited above — Montana Code Annotated — at "
    "<b>mca.legmt.gov</b>, the Legislature's own server, which publishes each "
    "section with a <i>History</i> line naming every session law that amended "
    "it. Read any <b>ARM</b> rule — Administrative Rules of Montana; the "
    "building, electrical, plumbing, and mechanical code adoptions live in "
    "<b>Title 24, chapter 301</b> — at <b>rules.mt.gov</b>, where the rule "
    "text is official and carries its effective date. Search that site by "
    "<i>rule number</i> rather than following a saved link: it is a search "
    "application, deep links collapse back to the search page, and a bad "
    "address returns an apparently working but empty page rather than an "
    "error. Two habits will keep you out of trouble in Montana. <b>First, "
    "always check whether the answer is in the MCA or the ARM</b>, and go "
    "read that one — Montana keeps an unusual share of what governs your "
    "house in the rules rather than the code, including real conditions on "
    "the homeowner exemptions that appear in no statute at all. "
    "<b>Second, watch which chapter you are in</b> — Montana splits one "
    "house across Title 50, chapter 60 (the building, plumbing, and "
    "electrical <i>permits</i>) and Title 37, chapters 68 and 69 (the "
    "electrician and plumber <i>licenses</i>), and an exemption written in "
    "one of them cannot waive a duty written in the other. All statute and "
    "rule text in this kit verified August 2026.")
