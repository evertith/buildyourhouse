"""Shared furniture and helpers for the South Carolina Owner-Builder Permit Kit.
...
SECTION = "South Carolina Owner-Builder Permit Kit"
HEADER_LEFT = "SOUTH CAROLINA OWNER-BUILDER PERMIT KIT"
...
# South Carolina cites by section symbol throughout ("S.C. Code Ann.
# § 40-59-260"), and platypus will happily break the line between the § and its
# number. NB is the non-breaking space to put after every § — and after a bare
# number that owns a unit ("50 feet", "200 sq ft"), for the same reason.
NB = "&#160;"


def sec(number):
    """'§ 40-59-260' with the space that can never break."""
    return f"§{NB}{number}"
...
def callout_long(title, paragraphs):
    ...
    short advisory. South Carolina prints its owner-builder disclosure
    statement in full inside § 40-59-260(C), and a 250pt callout arriving two
    thirds down a page throws the whole rest of the page away — several pages
    of pure whitespace across the kit. This rebuilds the same box as a
    LongTable that breaks between body paragraphs.
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. South Carolina is "
           "a mandatory-enforcement state: § 6-9-10 orders every municipality "
           "and county to enforce the state code, and to enforce <i>only</i> "
           "that code, so the question that dominates most states in this "
           "series — will anyone inspect this house? — has a statutory answer "
           "here. What South Carolina replaces it with is a stack of small "
           "obligations with hard consequences: a disclosure you sign in "
           "person, a notice you file at the register of deeds afterwards, "
           "an energy code fixed by statute rather than by the code council, "
           "and a coastline the law forbids anyone to describe county by "
           "county. Fees, portals, review queues and the extra permits your "
           "particular lot triggers still vary. Confirm every item with the "
           "office that will actually handle your parcel before you rely "
           "on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every South Carolina claim in this document "
                 f"({VERIFIED})",
...
STATUTE_NOTE = (
    "Everything cited here is free to read, in three places. Statutes "
    "(<b>S.C. Code Ann. § 40-59-260</b>) are at <b>scstatehouse.gov</b> → "
    "Code of Laws, and the whole chapter loads at one address you can type "
    "directly — <i>scstatehouse.gov/code/t40c059.php</i>, where <b>t40</b> is "
    "the Title and <b>c059</b> the chapter, zero-padded to three digits. "
    "Titles used here: <b>6</b> local government (building codes, the Energy "
    "Standard Act), <b>15</b> the eight-year repose, <b>29</b> mechanics' "
    "liens, <b>40</b> licensing, <b>44</b> health, <b>48</b> the coast. "
    "Regulations are at <b>scstatehouse.gov/coderegs/</b>, by chapter number. "
    "<b>One warning when you look something up:</b> a search engine will "
    "still hand you pages naming <b>DHEC</b>, an agency that ceased to exist "
    "in 2024 — check the masthead of any septic, well or coastal page you "
    "land on before you trust it. Read September 2026.")
