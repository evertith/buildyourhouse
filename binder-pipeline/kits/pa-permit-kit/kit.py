"""Shared furniture and helpers for the Pennsylvania Owner-Builder Permit Kit.
...
SECTION = "Pennsylvania Owner-Builder Permit Kit"
HEADER_LEFT = "PENNSYLVANIA OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Pennsylvania's UCC exclusion list and the seven-part
    recreational cabin test are long enough that a 250pt callout arriving two
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Pennsylvania is "
           "not a no-code state and it is not a no-inspection state: the "
           "Uniform Construction Code applies to every house in the "
           "Commonwealth, and five inspections are required on every one of "
           "them. What changes from one municipality to the next is <i>who "
           "performs them</i> — a municipal code official, an agency the "
           "municipality retained, or, where the municipality elected not to "
           "administer the code at all, an agency <b>you</b> hire yourself. "
           "Fees, review queues and the local permits stacked on top of the "
           "building permit vary too. Confirm every item with the office that "
           "will actually handle your parcel before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Pennsylvania claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Every source cited here is free to read, but Pennsylvania keeps statutes "
    "and regulations on <b>two different websites</b>. A citation reading "
    "<b>Pa. Code</b> is a <i>regulation</i> — <b>pacodeandbulletin.gov</b>, by "
    "title and chapter. A citation reading <b>P.S.</b> is a <i>statute</i>, "
    "and it is <b>not on that site at all</b>; the Pa. Code home page says so "
    "outright. Statutes are at <b>legis.state.pa.us</b> → Unconsolidated "
    "Statutes, by year and act number — the Construction Code Act is Act 45 "
    "of 1999. <b>One warning:</b> every Pa. Code chapter prints the "
    "<i>Pennsylvania Bulletin</i> issue it is current through, and an "
    "amendment published later is already law. Read that line first. All text "
    "here was read at those sources, September 2026.")
