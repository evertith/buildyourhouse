#!/usr/bin/env python3
"""Output audit for the PENNSYLVANIA Permit Kit PDFs.
...
Catches the four defects that survive a clean build but ruin a printed page:
  ...
  - words broken mid-token by a too-narrow table column ("pacodeandbulletin.g /
    ov", "Phot / os"). The detector looks for a very short lowercase fragment
...
OUT = os.path.join(HERE, "out", "pa-permit-kit")
...
def audit(pdf):
    ...
    for i, ws in enumerate(pages, start=1):
        ...
        is_cover = name.startswith("PA.0") and i == 1
        ...
        for y, row in sorted(rows_by_y.items()):
            ...
            # opens a line all over this kit, which quotes the Pa. Code
