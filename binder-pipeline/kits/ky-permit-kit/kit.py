"""Shared furniture and helpers for the Kentucky Owner-Builder Permit Kit.
...
SECTION = "Kentucky Owner-Builder Permit Kit"
HEADER_LEFT = "KENTUCKY OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Kentucky's statutory quotations run long enough that a
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Kentucky writes one "
           "mandatory residential code for the whole state — and then leaves "
           "the building permit, the inspections and the certificate of "
           "occupancy to each of its 120 counties and its cities, some of "
           "which require them and many of which do not. The state plumbing "
           "permit and the electrical certificate of approval apply either "
           "way. Confirm every item with the office that will actually handle "
           "your parcel before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Kentucky claim in this document "
                 f"({VERIFIED})",
...
STATUTE_NOTE = (
    ...
    "<b>apps.legislature.ky.gov</b> — Kentucky Law, then <b>Kentucky Revised "
    "Statutes</b> → KRS Title Page for a <b>KRS</b> section (chapters used "
    "here: <b>198B</b> the building code, <b>227A</b> electrical, <b>318</b> "
    "plumbing), or <b>Kentucky Administrative Regulations</b> → KAR List by "
    "Title for a <b>KAR</b> regulation (<b>815</b> Housing, Buildings and "
    "Construction; <b>902</b> Public Health; <b>401</b> Energy and "
    "Environment). <b>One warning when you check a number:</b> Kentucky prints "
    "an amended regulation with the new wording immediately followed by the "
    "wording it replaced, so a fee can appear on screen as \"fifty (50) "
    "forty-five (45) dollars.\" The <i>first</i> figure is the one in force — "
    "confirm it against the last \"eff.\" date on the HISTORY line at the foot "
    "of the regulation. All text here was read at that source, August 2026.")
