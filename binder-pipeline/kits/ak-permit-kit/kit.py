"""Shared furniture and helpers for the Alaska Owner-Builder Permit Kit.
...
SECTION = "Alaska Owner-Builder Permit Kit"
HEADER_LEFT = "ALASKA OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Alaska's exemption statute runs to a single 300-word
    sentence, and a 250pt callout arriving two thirds down a page throws the
    whole rest of the page away — several pages of pure whitespace across the
    kit. This rebuilds the same box as a LongTable that breaks between body
    paragraphs.
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Alaska has no "
           "statewide residential building code and, over most of its land "
           "area, no local building department either — so what binds your "
           "house is a short stack of <i>state</i> rules that apply whether "
           "or not anyone inspects, plus whatever your borough, your city, "
           "your lender and your utility require on top. Confirm every item "
           "with the office that will actually decide it before you rely "
           "on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Alaska claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any statute or regulation cited above free at <b>akleg.gov</b> → "
    "Bills &amp; Laws → Alaska Statutes for an <b>AS</b> citation, or → "
    "Alaska Administrative Code for an <b>AAC</b> citation. Both are "
    "searchable by number — type <i>08.18.161</i> or <i>18 AAC 72</i> into "
    "the section box. Every quotation here was read there and verified "
    "August 2026 against the Alaska Statutes 2025 compilation.")
