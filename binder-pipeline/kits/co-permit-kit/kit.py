"""Shared furniture and helpers for the Colorado Owner-Builder Permit Kit.
...
SECTION = "Colorado Owner-Builder Permit Kit"
HEADER_LEFT = "COLORADO OWNER-BUILDER PERMIT KIT"
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Colorado adopts no "
           "statewide residential building code: each county and each "
           "municipality decides for itself whether to require a building "
           "permit and which code edition to enforce, and a few counties "
           "require no building permit at all. Electrical and plumbing run on "
           "a separate, statewide track. Confirm every item with the office "
           "that will actually issue it before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Colorado claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any C.R.S. section cited above at <b>leg.colorado.gov</b> → Laws → "
    "Colorado Revised Statutes, which serves the official titles as PDFs. "
    "Read any rule cited as a <b>CCR</b> number — the State Electrical "
    "Board's rules are 3 CCR 710-1, the State Plumbing Board's are "
    "3 CCR 720-1 — in the Code of Colorado Regulations at "
    "<b>sos.state.co.us/CCR</b>, where the PDF is the official version and "
    "every past version is listed with its effective date. Beware "
    "third-party statute mirrors: they lag the session laws, and Colorado "
    "renumbered its entire professions title in 2019, so older guides cite "
    "sections that no longer exist. All statute and rule text in this kit "
