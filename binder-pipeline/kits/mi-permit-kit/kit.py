"""Shared furniture and helpers for the Michigan Owner-Builder Permit Kit.
...
SECTION = "Michigan Owner-Builder Permit Kit"
HEADER_LEFT = "MICHIGAN OWNER-BUILDER PERMIT KIT"
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Michigan's "
           "construction code is a single statewide code, but it is "
           "administered by more than 1,800 separate units of government — "
           "and your township, city, county, or the state Bureau of "
           "Construction Codes may each enforce a different trade on the "
           "same parcel. Confirm every item with the enforcing agency that "
           "will actually issue your permit before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Michigan claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any MCL section cited above free at <b>legislature.mi.gov</b> → "
    "Laws → MCL Search, or directly at legislature.mi.gov/Laws/"
    "MCL?objectName=mcl-339-2403 with the chapter and section swapped into "
    "the last two number groups — mcl-125-1510, mcl-339-2012, and so on. "
    "All statute text in this kit was read at that source and verified "
    "August 2026.")
