"""Shared furniture and helpers for the Wisconsin Owner-Builder Permit Kit.
...
SECTION = "Wisconsin Owner-Builder Permit Kit"
HEADER_LEFT = "WISCONSIN OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Wisconsin's statutory quotations run long enough that a
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Wisconsin writes "
           "one Uniform Dwelling Code that binds every new one- and two-family "
           "dwelling in the state, and then splits the <i>enforcement</i> of "
           "it three ways — a municipality that inspects with its own staff, a "
           "municipality that contracts the work out, and a town that has "
           "never taken it up, where the state's own certified agency does it "
           "instead. Your septic permit comes from the county no matter which "
           "of the three you are in. Confirm every item with the office that "
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Wisconsin claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Everything cited in this kit is free to read at "
    "<b>docs.legis.wisconsin.gov</b>, and the site takes a citation "
    "directly as a URL. For a statute — a <b>Wis. Stat.</b> section such as "
    "101.65 — open <b>docs.legis.wisconsin.gov/statutes/statutes/101</b> and "
    "find the section in the chapter. For a rule — a <b>Wis. Admin. Code</b> "
    "chapter such as SPS 321 or NR 812 — open "
    "<b>docs.legis.wisconsin.gov/code/admin_code</b> and pick the agency: "
    "<b>SPS</b> is Safety and Professional Services, which writes the Uniform "
    "Dwelling Code and the septic code, and <b>NR</b> is Natural Resources, "
    "which writes the well code and shoreland zoning. <b>One habit worth "
    "keeping when you check a number:</b> every chapter page prints its "
    "register date at the foot, in the form “Register <i>month year</i> "
    "No. <i>nnn</i>”. That date, not the year in the chapter title, tells you "
    "how current the text on your screen is. All text here was read at that "
