#!/usr/bin/env python3
"""Output audit for the WISCONSIN Permit Kit PDFs.
...
Catches the four defects that survive a clean build but ruin a printed page:
  ...
  - words broken mid-token by a too-narrow table column ("commerce.alaska.g /
    ov", "Phot / os"). The Alaska kit found two of these by eye; the detector looks
...
# Wisconsin cites its law as "s. 101.654" and "ss. SPS 305.31, 305.315", and
# quotes subsections as "(d)", "(bn),". Those wrap to the head of a line
# constantly in this kit and every one of them tripped the split-word test.
# A word broken by a narrow column is always pure letters — the renderer
# splits between glyphs, so the tail can never acquire a period or a bracket.
# Requiring the raw token to be punctuation-free separates the two cleanly
# without weakening the detector: "ov" from a broken URL still fires.
CITATION_PUNCT = set(".()[]§")
...
def audit(pdf):
    ...
    for i, ws in enumerate(pages, start=1):
        ...
        is_cover = name.startswith("WI.0") and i == 1
