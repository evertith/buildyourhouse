#!/usr/bin/env python3
"""Output audit for the TENNESSEE Permit Kit PDFs.
...
Catches the four defects that survive a clean build but ruin a printed page:
  ...
  - words broken mid-token by a too-narrow table column ("tn.gov/commerce/f /
    ire", "Phot / os"). The detector looks for a very short lowercase fragment
    starting a line, which is what the tail of a broken word always looks like
    and what ordinary prose never does.
...
# "am" was missing from the inherited list and fired on a real line of the
# permit record — "…if one is issued where I / am building:". It is as ordinary
# a line-opener as "is" or "it" once a checklist writes in the first person,
# which this kit does throughout.

# A quoted statutory subsection marker opening a line: "(b)", (c), (1), (iii).
# Optionally preceded by an opening quote, since a quotation often starts on
# one. Matched BEFORE the short-fragment test, and only ever as a whole token.
SUBSECTION = re.compile(r'^[“"\']?\([0-9a-zA-Z]{1,4}\)[.,;:]?$')
...
def audit(pdf):
    ...
    for i, ws in enumerate(pages, start=1):
        ...
        is_cover = name.startswith("TN.0") and i == 1
        ...
        for y, row in sorted(rows_by_y.items()):
            row.sort()
            first = row[0][1] or ""
            # A statutory subsection marker \u2014 "(b)", "(c)", "(1)" \u2014 legitimately
            # opens a line all over this kit, which quotes Tennessee Code
            # subsection by subsection. It is never a split word's tail: the
            # fragment left behind by a narrow column has no parentheses
            # wrapped round it, so requiring BOTH brackets keeps the detector
            # honest while silencing the quotations.
            if SUBSECTION.match(first):
                continue
            bare = first.strip(".,;:()\u201c\u201d\"'")
