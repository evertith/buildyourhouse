"""Shared furniture and helpers for the Arkansas Owner-Builder Permit Kit.
...
SECTION = "Arkansas Owner-Builder Permit Kit"
HEADER_LEFT = "ARKANSAS OWNER-BUILDER PERMIT KIT"
...
# Arkansas cites by section symbol far more than Kentucky did ("Ark. Code Ann.
# § 14-56-202"), and platypus will happily break the line between the § and its
...
def sec(number):
    """'§ 14-56-202' with the space that can never break."""
    return f"§{NB}{number}"
...
def callout_long(title, paragraphs):
    ...
    short advisory. Arkansas's statutory quotations run long enough that a
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Arkansas adopts one "
           "building code for the whole state and forbids your city or county "
           "from adopting a different one — and then requires nobody to "
           "enforce it on a house. The building permit, the inspections and "
           "the certificate of occupancy exist only where a city or a county "
           "chose to create them, and across most unincorporated Arkansas "
           "nobody did. What does not depend on that choice is the code "
           "itself, the onsite wastewater permit, and the trade licensing "
           "rules. Confirm every item with the office that will actually "
           "handle your parcel before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Arkansas claim in this document "
                 f"({VERIFIED})",
...
STATUTE_NOTE = (
    ...
    "(<b>Ark. Code Ann. § 14-56-202</b>) are at <b>arkleg.state.ar.us</b> "
    "under Arkansas Code — search the section number; the site hands off to a "
    "free Lexis viewer, so the address is not worth bookmarking. Agency rules "
    "are at <b>codeofarrules.arkansas.gov</b>, by Title: <b>012</b> Public "
    "Safety holds the Fire Prevention Code, <b>007</b> Health the wastewater "
    "rules. The building code reads free at "
    "<b>codes.iccsafe.org/codes/arkansas</b> — Volume III is residential. "
    "<b>One warning:</b> Act 841 of 2023 moved the State Fire Marshal out of "
    "the Arkansas State Police and repealed most of §§ 12-13-102 to 12-13-119, "
    "re-enacting those duties at <b>§ 20-22-1009</b>. Guides written before "
    "2023 still cite the repealed sections. Read September 2026.")
