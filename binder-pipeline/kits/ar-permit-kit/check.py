#!/usr/bin/env python3
"""Output audit for the ARKANSAS Permit Kit PDFs.
...
Catches the four defects that survive a clean build but ruin a printed page:
  ...
  - words broken mid-token by a too-narrow table column ("healthy.arkansas.g /
...
OUT = os.path.join(HERE, "out", "ar-permit-kit")
...
def audit(pdf):
    ...
    for i, ws in enumerate(pages, start=1):
        ...
        is_cover = name.startswith("AR.0") and i == 1
        ...
        for y, row in sorted(rows_by_y.items()):
            ...
            # opens a line all over this kit, which quotes Arkansas Code
