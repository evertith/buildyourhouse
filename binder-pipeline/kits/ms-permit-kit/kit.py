"""Shared furniture and helpers for the Mississippi Owner-Builder Permit Kit.
...
SECTION = "Mississippi Owner-Builder Permit Kit"
HEADER_LEFT = "MISSISSIPPI OWNER-BUILDER PERMIT KIT"
...
def callout(title, paragraphs):
    ...
    Use this for anything short enough to keep whole; callout_long otherwise.
    """
...
def callout_long(title, paragraphs):
    """A callout that MAY split, repeating its border but not its title.

    Mississippi's opt-out and coastal-wind provisions run long enough that a
    250pt KeepTogether arriving two thirds down a page throws the rest of the
    page away. This rebuilds the same box as a LongTable that breaks between
    body paragraphs. NOSPLIT glues the title row to the first paragraph so a
    title can never strand at the foot of a page; repeatRows is deliberately 0
    because a repeated advisory title reads as a duplicated box.
    """
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Mississippi is the "
           "hardest state in the country to write a permit guide for, because "
           "the answer genuinely changes at the county line: some counties "
           "enforce a building code and inspect your house, and others "
           "enforce none at all. Confirm every item with the office that will "
           "actually issue your permit before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Mississippi claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


# Points only at sources confirmed reachable in August 2026. The commercial
# statute mirrors (Justia, FindLaw, Casetext) all refused automated access
# when this kit was built, so the kit does not send buyers to them.
STATUTE_NOTE = (
    "Every Mississippi Code section cited above is free to read. The "
    "Legislature's official copy is at <b>legislature.ms.gov</b> → "
    "Mississippi Code (Title 73, Chapter 59 is residential builders; Title "
    "17, Chapter 2 is building codes; Title 41, Chapter 67 is onsite "
    "wastewater). The State Board of Contractors publishes the whole "
    "residential law and its rules as one booklet at <b>msboc.us</b> → Laws, "
    "and the Department of Health publishes the wastewater law at "
    "<b>msdh.ms.gov</b>. Enacted bills — which is how the dates and opt-out "
    "windows in this kit were checked — are at "
    "<b>billstatus.ls.state.ms.us</b>. All statute text quoted in this kit "
    "was read at those sources and verified August 2026.")
