"""Shared furniture and helpers for the California Owner-Builder Permit Kit.
...
SECTION = "California Owner-Builder Permit Kit"
HEADER_LEFT = "CALIFORNIA OWNER-BUILDER PERMIT KIT"
...
def callout_long(title, paragraphs):
    ...
    short advisory. California's statutory quotations run long enough that a
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. California writes "
           "one building code for the whole state — Title 24 — but permits are "
           "issued by 58 counties and hundreds of incorporated cities, and "
           "state law lets each of them amend that code for local conditions. "
           "Confirm every item with the city or county building department "
           "that will issue your permit before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every California claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any section cited above at <b>leginfo.legislature.ca.gov</b> → "
    "California Law, or go straight to leginfo.legislature.ca.gov/faces/"
    "codes_displaySection.xhtml?lawCode=BPC&amp;sectionNum=7044 and swap in the "
    "code and section. Codes used here: <b>BPC</b> Business and Professions, "
    "<b>HSC</b> Health and Safety, <b>LAB</b> Labor, <b>PRC</b> Public "
    "Resources, <b>CIV</b> Civil, <b>WAT</b> Water, <b>EDC</b> Education. Each "
    "page prints its own amendment history — check it against this kit's "
