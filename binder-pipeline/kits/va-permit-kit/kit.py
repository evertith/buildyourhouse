"""Shared furniture and helpers for the Virginia Owner-Builder Permit Kit.
...
SECTION = "Virginia Owner-Builder Permit Kit"
HEADER_LEFT = "VIRGINIA OWNER-BUILDER PERMIT KIT"
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Virginia has one "
           "statewide building code, but every county, city, and town "
           "enforces it through its own local building department, with its "
           "own forms, fees, and portal. Confirm every item with the "
           "building department that will issue your permit before you rely "
           "on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Virginia claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any Code of Virginia section cited above at "
    "<b>law.lis.virginia.gov</b> → Code of Virginia → by section, or "
    "directly at law.lis.virginia.gov/vacode/54.1-1101/ with the section "
    "number swapped in. Virginia Administrative Code sections (13VAC5-63 "
    "and the like) live under law.lis.virginia.gov/admincode. All statute "
    "and regulation text verified August 2026.")
