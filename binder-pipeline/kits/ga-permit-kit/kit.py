"""Shared furniture and helpers for the Georgia Owner-Builder Permit Kit.
...
SECTION = "Georgia Owner-Builder Permit Kit"
HEADER_LEFT = "GEORGIA OWNER-BUILDER PERMIT KIT"
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. In Georgia the "
           "state minimum codes bind statewide, but permits and inspections "
           "are a local option — each county or city decides its own forms, "
           "fees, and enforcement, and some enforce nothing at all. Confirm "
           "every item with the office that will issue your permit before "
           "you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Georgia claim in this document "
                 f"({VERIFIED})",
        hdr, body_rows, [CW - 2.15 * inch, 2.15 * inch], S, write_rows=False)


STATUTE_NOTE = (
    "Read any O.C.G.A. section cited above at <b>codes.findlaw.com/ga</b> — "
    "pattern: codes.findlaw.com/ga/title-43-professions-and-businesses/"
    "ga-code-sect-43-41-17/ with the title and section swapped in — or "
    "through the official O.C.G.A. portal linked from legis.ga.gov. Statute "
    "text was verified August 2026 against that mirror, whose stated "
    "currency was March 28, 2024; the 2024–2025 session laws (SB 503, "
    "HB 635, SB 125) postdate it. None is reported to change the "
    "owner-exemption text this kit quotes, but confirm exact wording at the "
    "official code before you rely on it.")
