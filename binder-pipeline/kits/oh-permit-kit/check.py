#!/usr/bin/env python3
"""Output audit for the OHIO Permit Kit PDFs.
...
Catches the four defects that survive a clean build but ruin a printed page:
  ...
  - words broken mid-token by a too-narrow table column ("odh.ohio.g /
...
def audit(pdf):
    ...
    for i, ws in enumerate(pages, start=1):
        ...
        is_cover = name.startswith("OH.0") and i == 1
        ...
        for y, row in sorted(rows_by_y.items()):
            ...
            # opens a line all over this kit, which quotes the Ohio Revised Code
