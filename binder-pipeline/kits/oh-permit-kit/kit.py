"""Shared furniture and helpers for the Ohio Owner-Builder Permit Kit.
...
SECTION = "Ohio Owner-Builder Permit Kit"
HEADER_LEFT = "OHIO OWNER-BUILDER PERMIT KIT"
...
# Ohio cites by section symbol constantly ("R.C. § 3781.10", "OAC 3701-29-06"),
# and platypus will happily break the line between the § and its number. NB is
# the non-breaking space to put after every § — and after a bare number that
# owns a unit ("50 feet", "200 square feet"), for the same reason.
NB = "&#160;"


def sec(number):
    """'§ 3781.10' with the space that can never break."""
    return f"§{NB}{number}"


def rule(number):
    """An Administrative Code rule number, kept on one line.

    Ohio rule numbers carry a colon and two hyphens ("4101:8-44-01") and
    reportlab will break them after either hyphen, which turns a citation into
    two half-numbers on two lines. The colon is the only safe break point and
    there is no good reason to take it, so the whole token is glued.
    """
...
def callout_long(title, paragraphs):
    """A callout that MAY split, repeating its border on the next page.

    callout() wraps design.callout_box in KeepTogether, which is right for a
    short advisory. Ohio's statutory quotations run long enough that a 250pt
    callout arriving two thirds down a page throws the whole rest of the page
    away — several pages of pure whitespace across the kit. This rebuilds the
    same box as a LongTable that breaks between body paragraphs.
    ...
    repeated advisory-box TITLE is not — it reads as a duplicated box. The
    continuation therefore carries the border alone, and the title row is glued
    to the first body paragraph so a title can never strand at a page foot.
...
def disclaimer(extra=None):
    ...
    txt = ("This is a process reference, not legal advice. Ohio writes one "
           "residential code for the whole state and applies it to your house "
           "by its own terms — and then, in that same code, excuses you from "
           "submitting plans, obtaining approvals, requesting inspections and "
           "getting a certificate of occupancy if no certified residential "
           "building department has jurisdiction over your parcel. What never "
           "depends on that is the code standard itself, the sewage treatment "
           "system permit, the private water system permit, and the contract "
           "and lien rules. Confirm every item with the office that will "
           "actually handle your parcel before you rely on it.")
    if extra:
        txt += " " + extra
...
def sources_table(rows, title=None):
    ...
    return d.titled_table(
        title or f"Sources — every Ohio claim in this document ({VERIFIED})",
...
STATUTE_NOTE = (
    "Everything cited here is free to read in one place. Ohio publishes both "
    "its statutes and its agency rules at <b>codes.ohio.gov</b> — the Revised "
    "Code (<b>R.C. § 3781.10</b>) under Ohio Revised Code, the Administrative "
    "Code (<b>OAC 4101:8-1-01</b>, <b>OAC 3701-29-06</b>) under Ohio "
    "Administrative Code. Both are searchable by number alone. <b>One warning "
    "about how the building code reads there:</b> the Board of Building "
    "Standards files its rules as PDFs of the amendment itself, so text that "
    "was deleted is struck through and text that was added is underlined, on "
    "the same line. Copy that line and the strikethrough vanishes — which is "
    "how a referenced standard reading \"70—<s>17</s><u>23</u>\" gets quoted "
    "as \"1723\" or, worse, as the edition Ohio just replaced. Read the rule "
    "PDF with your eyes, not your clipboard. Read September 2026.")
