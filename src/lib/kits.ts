...
export const STATE_KITS: StateKit[] = [
  ...
  K('Connecticut', 'ct', 'connecticut', { phase: 11 }),
  ...
  K('Hawaii', 'hi', 'hawaii', { phase: 11 }),
  K('Idaho', 'id', 'idaho', { phase: 5 }),
  K('Illinois', 'il', 'illinois', { phase: 8 }),
  K('Indiana', 'in', 'indiana', { phase: 9 }),
  K('Iowa', 'ia', 'iowa', { phase: 6 }),
  ...
  K('Maine', 'me', 'maine', { phase: 9 }),
  K('Maryland', 'md', 'maryland', { phase: 10 }),
  K('Massachusetts', 'ma', 'massachusetts', { phase: 7 }),
  ...
  K('Minnesota', 'mn', 'minnesota', { phase: 8 }),
  ...
  K('Missouri', 'mo', 'missouri', { phase: 8 }),
  ...
  K('Nebraska', 'ne', 'nebraska', { phase: 5 }),
  K('Nevada', 'nv', 'nevada', { phase: 7 }),
  ...
  K('North Dakota', 'nd', 'north-dakota', { phase: 6 }),
  K('Ohio', 'oh', 'ohio', {
    status: 'shipped',
    checkoutUrl: 'https://buy.stripe.com/14AcN54S3aoTfM8eZ5fAc0k',
    hook: 'The code section that excuses you from permits where no department is certified, 23 counties with no residential certification anywhere, a 2023 NEC change hidden behind a strikethrough, and state trade licenses that stop at your property line.',
  }),
  K('Oklahoma', 'ok', 'oklahoma', { phase: 10 }),
  K('Oregon', 'or', 'oregon', { phase: 9 }),
  K('Pennsylvania', 'pa', 'pennsylvania', {
    status: 'shipped',
    checkoutUrl: 'https://buy.stripe.com/3cI00jbgr9kPfM818ffAc0j',
    hook: 'The code applies to every house in the Commonwealth and five inspections are required on all of them — but in 119 municipalities nobody is assigned to do them until you hire an agency yourself, and you give up the right to appeal when you do.',
  }),
  K('Rhode Island', 'ri', 'rhode-island', { phase: 12 }),
  K('South Carolina', 'sc', 'south-carolina', {
    status: 'shipped',
    checkoutUrl: 'https://buy.stripe.com/cNi6oH1FR54zarO04bfAc0m',
    hook: 'A notice you file after the house is finished — skip it and the statute revokes the exemption you built under. Plus the $500 threshold that decides who you may hire, an energy code the code council has no power to move, and a beachfront setback measured in erosion rates.',
  }),
  K('South Dakota', 'sd', 'south-dakota', { phase: 11 }),
  K('Tennessee', 'tn', 'tennessee', {
    status: 'shipped',
    checkoutUrl: 'https://buy.stripe.com/9B6cN51FRfJd9nKdV1fAc0l',
    hook: 'Three enforcement regimes over one state, an opt-out that expires 180 days after the next election, the electrical permit that survives it in all 37 no-code counties, and the definition that turns every trade you hire into a prime contractor.',
  }),
  K('Texas', 'tx', 'texas', {
    status: 'shipped',
  ...
  K('Utah', 'ut', 'utah', { phase: 9 }),
  K('Vermont', 'vt', 'vermont', { phase: 11 }),
  ...
  K('West Virginia', 'wv', 'west-virginia', { phase: 7 }),
