...
// Real per-page dates from git history (generated by scripts/generate-content-dates.mjs
// via the prebuild hook) — a uniform build timestamp tells crawlers nothing.
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const toolsPages = [
    {
      url: `${baseUrl}/tools-and-equipment`,
      lastModified: currentDate,
      changeFrequency: 'monthly' as const,
      priority: 0.8,
    },
  ...
  return allPages.map((entry) => {
    ...
    return dates ? { ...entry, lastModified: new Date(dates.modified) } : entry;
  });
}
