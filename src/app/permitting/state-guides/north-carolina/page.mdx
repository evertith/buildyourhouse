...
<CodeAlertCapture state="North Carolina" />
...
You can produce that site plan yourself: the free [Site Plan Studio](/site-plan-studio) draws the lot and footprint to a standard engineering scale with the setbacks dimensioned, then prints it letter-size with a title block.
...
<DataTable
  caption="Well and septic separations — commonly cited figures; your county health department applies the governing rules (15A NCAC 18E for wastewater, 15A NCAC 18A .1700 for wells) to your parcel"
  headers={["Separation", "Commonly cited distance"]}
  rows={[
    ["Well to septic", "100 feet"],
    ["Well to property line", "10 feet"],
    ["Septic to property line", "Varies by system size and county"],
    ["Well to dwelling", "Ask your county — siting is set at the permit, not by a single statewide table"]
  ]}
/>

Treat that table as orientation, not authority: the binding numbers come from the state rules as your county health department applies them to your soil, system type, and lot — get the actual distances in writing during the site evaluation, before you fix the house location.
